...
struct segtree{
    ...
    int32_t find(int32_t v,int32_t l){
        return find(v,l,0,0,size);
    }

    int32_t find(int32_t v,int32_t l, int32_t x, int32_t lx, int32_t rx){
        if(data[x]<v||rx<=l){return -1;}
        if(rx-lx==1){
            return lx;
        }
        int32_t m = (lx+rx)/2;
        int32_t res = find(v,l,2*x+1,lx,m);
        if(res==-1){
            return find(v,l,2*x+2,m,rx);
        }
        return res;
    }
};
